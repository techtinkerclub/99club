...
function gen16(d,seed){var r=rngFromSeed(seed),p=choose(r,ANTONYMS.filter(function(x){return suitable(x[0],d)&&suitable(x[1],d);})),a=p[0],b=p[1],os=shuffle(r,ANTONYMS.filter(function(x){return x.indexOf(a)<0&&x.indexOf(b)<0;})),ga=shuffle(r,[a,os[0][0],os[1][0]]).slice(0,d==='easy'?2:3),gb=shuffle(r,[b,os[2][1],os[3][1]]).slice(0,d==='easy'?2:3);return makeQ('opposite_meaning',d,'Choose one word from each group that are opposite in meaning.  Group 1: '+ga.join(', ')+'   Group 2: '+gb.join(', '),a+' — '+b,{key:'ant:'+a+':'+b+':'+ga.join(',')+':'+gb.join(','),explanation:a+' and '+b+' are opposites.',check:{kind:'pair',a:a,b:b}});}
...
function gen18(d,seed){var r=rngFromSeed(seed),mode=d==='easy'?'sum':d==='standard'?choose(r,['sum','difference','double_sum']):choose(r,['difference','double_sum','product']),lim=mode==='product'?9:(d==='challenge'?30:18),pairs=Array.from({length:4},function(){return[randInt(r,2,lim),randInt(r,2,lim)];}),ex=pairs.slice(0,3).map(function(x){return x[0]+', '+x[1]+', '+rr(mode,x[0],x[1]);}),last=pairs[3],ans=rr(mode,last[0],last[1]);return makeQ('related_numbers',d,'Each group follows the same rule:  ('+ex.join(')   (')+')   ('+last[0]+', '+last[1]+', ?).  What is missing?',ans,{choices:shuffle(r,unique([ans,last[0]+last[1],Math.abs(last[0]-last[1]),ans+2])).map(String).slice(0,4),key:'rn:'+mode+':'+pairs.flat().join(','),explanation:'Apply the same '+mode.replace('_',' ')+' rule.',check:{kind:'related',mode:mode,a:last[0],b:last[1]}});}
function gen19(d,seed){var r=rngFromSeed(seed),p=shuffle(r,wordPool(d).filter(function(w){return w.length===4&&new Set(w).size===4;})),target=choose(r,p),need=new Set(target.split('')),ex=[];for(var i=0;i<p.length&&ex.length<2;i++){if(p[i]===target)continue;ex.push(p[i]);var covered=new Set(ex.join('').split('')),ok=true;need.forEach(function(ch){if(!covered.has(ch))ok=false;});if(ex.length===2&&ok)break;if(ex.length===2&&!ok)ex.pop();}if(ex.length<2)return null;var letters=unique(ex.concat([target]).join('').split('')),digits=shuffle(r,'1234567890'.split('')).slice(0,letters.length),map={};letters.forEach(function(l,i){map[l]=digits[i];});var enc=function(w){return w.split('').map(function(c){return map[c];}).join('');},ans=enc(target),wrong=unique([ans.split('').reverse().join(''),enc(target.slice(1)+target[0]),String((Number(ans)+11)%10000).padStart(4,'0')]).filter(function(x){return x!==ans;});return makeQ('word_number_codes',d,'Each letter always has the same digit.  '+ex[0]+' = '+enc(ex[0])+' and '+ex[1]+' = '+enc(ex[1])+'.  What is the code for '+target+'?',ans,{choices:shuffle(r,[ans].concat(wrong.slice(0,3))),key:'wn:'+letters.map(function(l){return l+map[l];}).join('')+':'+ex.join(':')+':'+target,explanation:'Use the digit assigned to each letter, in the target word order.',check:{kind:'wn',map:map,target:target}});}
