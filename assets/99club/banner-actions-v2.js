...
const PUPIL_FACING=location.pathname==='/play/'||location.pathname.startsWith('/practice/');
...
function bindContact(modal){
  ...
  form?.addEventListener('submit',async e=>{
    ...
    try{
      const response=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({name:name||'Not provided',email:reply,_replyto:reply,message,_subject:`99 Club Studio contact${name?` — ${name}`:''}`,_template:'table',_url:window.location.origin+window.location.pathname})});
...
function openKofi(opener){if(PUPIL_FACING)return;const root=ensureModals(),modal=root.querySelector('#tt99-kofi-modal');if(!modal)return;lastKofiOpener=opener||document.activeElement;modal.hidden=false;document.body.classList.add('tt99-kofi-open');ensureKofiPanel(root);setTimeout(()=>modal.querySelector('.tt99-kofi-close')?.focus(),0);}
...
function enhance(){ensureStyles();enhanceBrandTools();enhanceCustomHero();enhanceGuideHero();if(PUPIL_FACING)document.querySelectorAll('[data-tt99-kofi-open],.tt99-brand-tool--support').forEach(el=>el.remove());}
