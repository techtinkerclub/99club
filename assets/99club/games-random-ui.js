/* 99 Club Studio · Random pack UI v1.5.0
...
(function(global){
  ...
  const SCOPE_KEY=G.PACK_MODE.storageScopeKey||'tt99-games-random-scope-v1';
  ...
  function scopeLabel(value){return value==='verbal'?'Verbal Reasoning only':value==='mixed'?'Maths + Verbal Reasoning':'Maths games only';}
  ...
  function enhance(){
    ...
    const currentMode=mode(),currentCount=count(),currentPerPage=perPage(),currentDifficulty=difficulty(),currentScope=scope(),currentWeights=weights(),sheetCount=Math.ceil(currentCount/currentPerPage);
    ...
    if(grid){
      ...
      holder.innerHTML=`
        ...
        <div class="tt99-pack-options-v134 ${currentMode==='manual'?'is-manual':''}">
          ${currentMode==='random'?`<label class="tt99-field"><span>Content</span><select id="games-random-scope"><option value="maths" ${currentScope==='maths'?'selected':''}>Maths games only</option><option value="mixed" ${currentScope==='mixed'?'selected':''}>Maths + Verbal Reasoning</option><option value="verbal" ${currentScope==='verbal'?'selected':''}>Verbal Reasoning only</option></select><small>Maths-only remains the default so existing random packs do not change unexpectedly.</small></label><label class="tt99-field"><span>Difficulty</span><select id="games-random-difficulty"><option value="easy" ${currentDifficulty==='easy'?'selected':''}>Easy</option><option value="standard" ${currentDifficulty==='standard'?'selected':''}>Standard</option><option value="challenge" ${currentDifficulty==='challenge'?'selected':''}>Challenge</option><option value="mixed" ${currentDifficulty==='mixed'?'selected':''}>Mixed — weighted</option></select><small>${currentDifficulty==='mixed'?'Uses the mix below across the whole pack.':'Applied consistently across the random pack.'}</small></label>`:''}
    ...
    if(toolbar){
      ...
      if(spans[0]){
        const prefix=currentMode==='random'?(scopeLabel(currentScope)+' · '+(currentDifficulty==='mixed'?`Mixed ${currentWeights.easy}/${currentWeights.standard}/${currentWeights.challenge}`:difficultyLabel(currentDifficulty))):'Selected games';
