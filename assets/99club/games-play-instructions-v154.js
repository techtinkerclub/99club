...
const COPY={
  ...
  crossnumber:'Solve each clue and enter one digit per cell. Across answers run left to right and Down answers top to bottom; crossing answers must share the same digit.',
  ...
  arithmeticcages:'Fill the grid with 1–N, using each number once in every row and column. Each cage must make its target using the shown operation; for a two-cell − or ÷ cage, either order is allowed.',
  ...
  nonogram:'Use the row and column clues to shade the picture. Each clue number is one consecutive shaded block; separate multiple blocks by at least one empty cell and keep them in clue order.',
  numberpath:'Use every number in the sequence exactly once. Consecutive numbers must touch by a side — up, down, left or right — never diagonally.',
  ...
  takuzu:'Fill the grid with 0s and 1s. Every row and column needs equal numbers of each, no 000 or 111 may appear horizontally or vertically, and no two completed rows or columns may match.',
  ...
  hashi:'Connect every island into one network using horizontal or vertical bridges. Match each island number, use at most two bridges between a pair, and never cross bridges or pass through another island.',
  mathsmines:'Find exactly the stated number of gems. Each clue counts gems in its eight surrounding squares, including diagonals; a clue square itself cannot contain a gem.',
  ...
  diagonalpath:'Use every number from 1 to the final number exactly once. Consecutive numbers may touch by a side or a corner, and printed anchor numbers are fixed.',
  squaresearch:'Find every non-overlapping 2 × 2 block whose four numbers total the target. Correct target blocks never share a cell.',
  ...
  perimeterregions:'Divide every cell into one region by drawing grid-edge boundaries. Each region must contain exactly one clue, and that clue is its outside perimeter in unit grid edges — not its area.'
};
