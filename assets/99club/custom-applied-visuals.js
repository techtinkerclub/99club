...
for(let j=0;j<36;j++){const tenths=[25+((j*7)%75),18+((j*11)%62),30+((j*13)%70)],total=tenths.reduce((a,b)=>a+b,0),values=tenths.map(t=>t/10),answer=(total/10).toFixed(1).replace(/\\.0$/,'');
out.push(q('measure_scales','joined_lengths',i++,`Three blocks are joined end to end. Find their total length.`,`${answer} cm`,{key:`joined:${tenths.join('-')}`,visual:{type:'applied',subtype:'joined_blocks',values,total:total/10}}));}
return out;
}
...
if(v.subtype==='jug'){const left=x+w*.34,top=y+h*.12,jw=w*.30,jh=h*.70;C.rect(left,top,jw,jh,{stroke:ink,width:.8});const frac=v.level/v.max,waterH=jh*frac;C.rect(left,top+jh-waterH,jw,waterH,{fill,stroke:null});for(let val=0;val<=v.max;val+=v.step){const yy=top+jh-jh*val/v.max;C.line(left+jw,yy,left+jw+9,yy,{color:ink,width:.55});if(val>0)C.text(left+jw+13,yy+2,String(val),5.8,{color:muted});}C.text(x+w/2,top+jh+16,'ml',6.5,{bold:true,align:'center',color:teal});if(v.target)C.text(x+w*.18,top+jh*(1-v.target/v.max)+2,`target ${v.target}`,5.8,{color:teal});}
else if(v.subtype==='ruler'){const left=x+w*.10,right=x+w*.90,cy=y+h*.55;C.line(left,cy,right,cy,{color:ink,width:1});for(let k=0;k<v.ticks;k++){const xx=left+(right-left)*k/(v.ticks-1);C.line(xx,cy-10,xx,cy+2,{color:ink,width:.6});if(k%2===0)C.text(xx,cy+15,String(v.start+k*v.step),5.8,{align:'center',color:muted});}const mx=left+(right-left)*v.mark/(v.ticks-1);C.line(mx,cy-28,mx,cy-12,{color:teal,width:1.4});C.polygon([{x:mx-4,y:cy-18},{x:mx+4,y:cy-18},{x:mx,y:cy-10}],{fill:teal,stroke:teal,width:.3});}
else if(v.subtype==='joined_blocks'){const vals=v.values||[],sum=vals.reduce((a,b)=>a+Number(b),0)||1,left=x+w*.10,top=y+h*.36,totalW=w*.80,bh=Math.min(48,h*.34),muted2=[95,105,120];let xx=left;vals.forEach((n,j)=>{const ww=totalW*Number(n)/sum;C.rect(xx,top,ww,bh,{fill:j%2?fill:pale,stroke:ink,width:.7});C.text(xx+ww/2,top+bh*.58,`${n} cm`,6.4,{bold:true,align:'center',color:ink});xx+=ww;});C.line(left,top+bh+15,left+totalW,top+bh+15,{color:muted2,width:.6});C.text(left+totalW/2,top+bh+27,answers?`${v.total} cm`:'total = ?',6.8,{bold:true,align:'center',color:answers?teal:muted2});}
else if(v.subtype==='timeline'){const blocks=v.blocks||[],start=blocks[0]?.start||0,end=blocks[blocks.length-1]?.end||start+1,left=x+w*.08,right=x+w*.92,top=y+h*.35,bh=42;blocks.forEach((b,j)=>{const xx=left+(right-left)*(b.start-start)/(end-start),ww=(right-left)*(b.end-b.start)/(end-start);C.rect(xx,top,ww,bh,{fill:j===v.ask?fill:pale,stroke:line,width:.5});C.text(xx+ww/2,top+17,b.name,5.8,{bold:true,align:'center',color:j===v.ask?teal:ink});C.text(xx,top+bh+13,timeText(b.start),5.8,{align:'center',color:muted});if(j===blocks.length-1)C.text(xx+ww,top+bh+13,timeText(b.end),5.8,{align:'center',color:muted});});}
