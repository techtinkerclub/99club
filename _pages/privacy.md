    ...
    <section class="page__content">
      <div class="tt99-guide">
        ...
        <section class="tt99-guide-section tt99-guide-highlight">
          ...
          <p>When a school name has been entered in Studio, consented product events can include a stable <strong>opaque school key</strong> randomly created and remembered on the staff browser. The school name itself is not sent to Google Analytics. When the browser supplies an external referrer, Studio can also include the <strong>referring website origin</strong> (for example <code>https://school.example</code>). The path, query string and fragment of the referring page are deliberately discarded.</p>
        ...
        <section class="tt99-guide-section">
          ...
          <p>The dedicated 99 Club parent-practice page at <code>/practice/</code> and puzzle-practice page at <code>/practice/puzzles/</code> are separate from the main Studio interfaces. Their links carry the teaching/generation settings needed to reproduce the school-selected practice. If a school name has been entered when the link is created, the link can also carry a random opaque school-level key created on the staff browser. The school name itself is not placed in the parent URL.</p>
