...
try{
  ...
  for(const token of ['function clockTool','function angleFromPointer','function setHandFromPointer','function startDrag','data-cl-hand="hour"','data-cl-hand="minute"','data-cl-readout="24"','data-cl-readout="12"','Roman numerals I–XII','Toggle am / pm','Hand snapping'])if(!goodiesToolsA.includes(token))fail('goodies-clock','Clock direct-manipulation workbench missing: '+token);
  for(const token of ['.gd-clock-tick','.gd-clock-hand-hit','.gd-clock-hand-hit--hour','.gd-clock-hand-hit--minute','.gd-clock-readouts'])if(!goodiesCss.includes(token))fail('goodies-clock','Clock workbench styling missing: '+token);
  ok('goodies-clock','Clock supports direct hour/minute hand dragging, keyboard adjustment, 1/5-minute precision, Roman numerals and live 12/24-hour readouts');
