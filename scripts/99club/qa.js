...
if(!shareCodecSrc.includes('/^(?:\\/play\\/?|\\/tools\\/99-club\\/games\\/play\\/?)$/'))fail('challenge-link-codec','Share codec does not recognise the current /play/ path');
else ok('challenge-link-codec','Share codec recognises current and legacy Online Play paths before creating /c/ challenge URLs');
...
try{
  ...
  for(const phrase of [
    ...
    'Not an official Radford Semele school webpage',
    '1 · Buttons',
    '2 · Cards',
    '3 · Widgets',
  ...
  ])if(!demo.includes(phrase))fail('school-demo','Single school demo missing: '+phrase);
  ...
  ok('school-demo','Single three-tab platform-neutral school integration demo checked');
