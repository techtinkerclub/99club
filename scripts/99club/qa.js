...
try{
  ...
  for(const token of [
    ...
    "{id:'fdp-equivalence'",
    "{id:'fraction-sequence'",
    "{id:'fraction-jump'",
    "{id:'compare-fractions'",
    "{id:'zoom-read'",
    "{id:'double-line-value'",
    ...
    'data-nl-scale-mode="zoom"',
    'data-nl-scale-mode="linked"',
    'nl-new-line-mode',
    'nl-line-min',
  ...
  for(const token of ["{id:'read-number'","{id:'digit-value'","{id:'build-number'","{id:'non-standard'","{id:'zero-placeholder'",'function customAnswerSources','function updateChallengeAnswer','data-pv-workflow','pv-custom-answer-source','CK.tabsHtml','function exportControlsHtml','function boardExportSvg','data-pv-export','function exportTargetSvg','pv-copy-image','pv-svg-download','pv-response-lines'])if(!goodiesToolsA.includes(token))fail('goodies-place-value','Place Value shared challenge/export contract missing: '+token);
  ...
  for(const token of ["{id:'find-length'","{id:'find-perimeter'","{id:'find-area'","{id:'perimeter-area'","{id:'build-area'","{id:'area-perimeter-units'",'function customAnswerSources','function resolveAnswerSource','function generateChallenge','data-ge-workflow','ge-custom-answer-source','function bindChallengeStageActions','CK.pickerHtml','CK.bannerHtml','hiddenMetrics','function exportControlsHtml','function exportMetricHidden','function geoboardExportSvg','function exportTargetSvg','data-ge-export','data-ge-export-mode','ge-copy-image','ge-svg-download','ge-response-lines'])if(!goodiesToolsB.includes(token))fail('goodies-geoboard','Geoboard shared challenge contract missing: '+token);
  ok('goodies-geoboard','Geoboard keeps direct drag/undo geometry and adds shared Standard/Custom live-measurement challenges plus reusable vector exports');
  for(const token of ["{id:'read-coordinate'","{id:'plot-coordinate'","{id:'missing-coordinate'","{id:'reflect-axis'","{id:'translate-point'","{id:'identify-quadrant'",'function customAnswerSources','function resolveCustomAnswerSource','function challengeFrozen','function generateChallenge','data-co-workflow','co-custom-answer-source','function bindChallengeStageActions','CK.pickerHtml','CK.bannerHtml','hiddenPointLabels','pointLabelOverrides','fourQuadrants','function exportControlsHtml','function exportPointHidden','function coordinateExportSvg','function exportTargetSvg','data-co-export','data-co-export-mode','co-copy-image','co-svg-download','co-response-lines'])if(!goodiesToolsB.includes(token))fail('goodies-coordinates','Coordinates shared challenge contract missing: '+token);
  ok('goodies-coordinates','Coordinates keeps direct point dragging and persistent quadrant switching while adding pupil-safe Standard/Custom challenges and reusable vector exports');
  for(const token of ["{id:'read-mark'","{id:'place-mark'","{id:'distance-between'","{id:'cm-to-mm'","{id:'mm-to-cm'","{id:'cm-to-m'","{id:'unit-misconception'",'function customAnswerSources','function liveAnswer','function markerFrozen','function generateChallenge','data-me-workflow','me-custom-answer-source','function bindRuler','function bindChallengeStageActions','CK.pickerHtml','CK.bannerHtml','secondaryCm','function exportControlsHtml','function exportReadoutHidden','function rulerExportSvg','function exportTargetSvg','data-me-export','data-me-export-mode','me-copy-image','me-svg-download','me-response-lines'])if(!goodiesToolsB.includes(token))fail('goodies-measurement','Measurement shared challenge contract missing: '+token);
  for(const token of ['.gd-ruler.is-interactive','.gd-ruler-marker.is-interactive','.gd-ruler-marker--secondary','.gd-ruler-marker-label'])if(!goodiesCss.includes(token))fail('goodies-measurement','Direct Measurement ruler styling missing: '+token);
  ok('goodies-measurement','Measurement ruler supports direct touch/keyboard placement, live conversions, shared Standard/Custom challenges and reusable vector exports');
  for(const token of ['function arrayBuilder','function partitionMath','data-ab-resize="cols"','data-ab-resize="rows"','data-ab-row-split','data-ab-col-split','data-ab-partial','Rotate / swap factors','Repeated addition','Related division facts','Partial products',"{id:'count-total'","{id:'multiplication-fact'","{id:'missing-factor'","{id:'related-division'","{id:'build-array'","{id:'commutative-fact'","{id:'partial-products'",'function customAnswerSources','function resolveAnswerSource','function challengeFrozen','function generateChallenge','data-ab-workflow','ab-custom-answer-source','function bindChallengeStageActions','CK.pickerHtml','CK.bannerHtml','hiddenDimensions','hiddenEquation','targetRows','targetCols','function exportControlsHtml','function exportHiddenFlag','function exportHiddenDimension','function arrayExportSvg','function exportTargetSvg','data-ab-export','data-ab-export-mode','ab-copy-image','ab-svg-download','ab-response-lines','data-ab-export-build-grid'])if(!goodiesToolsA.includes(token))fail('goodies-arrays','Array direct-manipulation workbench missing: '+token);
  for(const token of ['.gd-array-workbench','.gd-array-board','.gd-array-resize--cols','.gd-array-resize--rows','.gd-array-cell.is-row-split','.gd-array-cell.is-col-split','.gd-array-partial'])if(!goodiesCss.includes(token))fail('goodies-arrays','Array workbench styling missing: '+token);
  ok('goodies-arrays','Arrays supports direct row/column resizing, factor rotation, partitions, repeated addition, inverse division facts, partial products, shared Standard/Custom challenges and reusable vector exports');
  for(const token of ['function clockTool','function angleFromPointer','function setHandFromPointer','function startDrag','data-cl-hand="hour"','data-cl-hand="minute"','data-cl-readout="24"','data-cl-readout="12"','Roman numerals I–XII','Toggle am / pm','Hand snapping',"{id:'read-five'","{id:'read-minute'","{id:'roman-read'","{id:'set-five'","{id:'set-minute'","{id:'elapsed-forward'","{id:'twelve-to-twentyfour'","{id:'twentyfour-to-twelve'","{id:'time-language'","{id:'hour-hand-misconception'",'function resolveAnswerSource','function handsFrozen','function generateChallenge','data-cl-workflow','cl-custom-answer-source','function bindChallengeStageActions','CK.pickerHtml','CK.bannerHtml','hiddenReadouts','targetHour','targetMinute','startHour','startMinute','function exportControlsHtml','function exportReadoutHidden','function exportClockState','function clockExportSvg','function exportTargetSvg','data-cl-export','data-cl-export-mode','cl-copy-image','cl-svg-download','cl-response-lines','data-cl-export-set-blank'])if(!goodiesToolsA.includes(token))fail('goodies-clock','Clock direct-manipulation workbench missing: '+token);
  for(const token of ['.gd-clock-tick','.gd-clock-hand-hit','.gd-clock-readouts'])if(!goodiesCss.includes(token))fail('goodies-clock','Clock workbench styling missing: '+token);
  ok('goodies-clock','Clock supports direct hand manipulation, 1/5-minute precision, Roman numerals, live 12/24-hour readouts, shared pupil-safe Standard/Custom challenges and reusable vector exports');
