    ...
    async function drawerTest(){
      try{
        ...
        if(compact){
          ...
          const reserve=parseFloat(getComputedStyle(board).getPropertyValue('--tt99-context-pad-space'))||0,padH=pad.getBoundingClientRect().height,marginBottom=parseFloat(getComputedStyle(board).marginBottom)||0;
          if(!board.classList.contains('tt99-context-pad-reserve'))fail('keypad-clearance','expanded mobile keypad did not add reserve space after the puzzle');
          if(reserve<padH+12)fail('keypad-clearance','reserved mobile keypad space is too small: reserve='+Math.round(reserve)+' pad='+Math.round(padH));
          if(marginBottom<padH)fail('keypad-clearance','expanded keypad reserve is not participating in document flow: margin='+Math.round(marginBottom)+' pad='+Math.round(padH));
        }else{
        ...
        const handle=pad.querySelector('.tt99-context-pad-handle'),toggle=pad.querySelector('.tt99-context-pad-toggle');if(!handle||!toggle)fail('keypad-drawer','drawer handle/toggle missing');else{toggle.click();await sleep(40);if(!pad.classList.contains('tt99-context-pad-collapsed'))fail('keypad-drawer','toggle did not collapse drawer');if(compact&&board.classList.contains('tt99-context-pad-reserve'))fail('keypad-clearance','collapsed keypad still reserves its expanded height');toggle.click();await sleep(60);if(pad.classList.contains('tt99-context-pad-collapsed'))fail('keypad-drawer','toggle did not reopen drawer');if(compact&&!board.classList.contains('tt99-context-pad-reserve'))fail('keypad-clearance','reopened mobile keypad did not restore puzzle clearance');}
        ...
        const outside=board.querySelector('.tt99-conn-card header')||document.body;outside.click();await sleep(40);if(pad.classList.contains('tt99-context-pad-active'))fail('keypad-drawer','outside tap did not dismiss drawer');if(board.classList.contains('tt99-context-pad-reserve'))fail('keypad-clearance','closing keypad left stale puzzle reserve space');
