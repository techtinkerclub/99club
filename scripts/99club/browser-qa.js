  ...
  (function(){
    ...
    async function sharePanelTest(){
      try{
        ...
        if(!payload||!Array.isArray(payload.files)||payload.files.length!==1||!payload.text||!/\/c\/\?c=/.test(payload.url||''))fail('share-panel','card-plus-link payload is missing image, caption or exact puzzle URL');
