  ...
  (function(){
    ...
    async function safeFitLayoutTest(){
      try{
        ...
        settings.open=false;
        if(Math.abs(after-before)<=4&&htmlOverflow!=='hidden'&&bodyOverflow!=='hidden'&&(!transform||transform==='none'))pass('safe-fit','compact play layout keeps settings as an overlay without page-locking or board transforms');
